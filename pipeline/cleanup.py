...
def provenance_source_id(path: Path, source_url: str) -> str:
    digest = hashlib.sha256((source_url or path.name).encode("utf-8")).hexdigest()[:12]
...
def main() -> int:
    ...
    provenance_dir = shield_root / "var" / "intake" / TYPE / "provenance" / "current"
    ...
    ensure_directory(provenance_dir)
    ...
    for path in inputs:
        ...
        metadata = parse_metadata(path.with_suffix(".meta"))
        source_url = metadata.get("source_url", "")
        source_id = provenance_source_id(path, source_url)
        domains_file = f"{source_id}-domains.txt"
        ips_file = f"{source_id}-ips.txt"
        file_domains = file_valid_entries - file_ips
        write_sorted_lines(provenance_dir / domains_file, file_domains)
        write_sorted_lines(provenance_dir / ips_file, file_ips)
        provenance_rows.append(
            "\t".join(
                [
                    ...
                    domains_file,
                    ips_file,
                    str(len(set(file_domains))),
                    str(len(set(file_ips))),
    ...
    provenance_manifest = provenance_dir / "manifest.tsv"
